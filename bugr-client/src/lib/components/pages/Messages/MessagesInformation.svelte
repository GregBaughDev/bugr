<script lang="ts">
  ...
  // can get the below ourselves from store - TO DO : REMOVE
...
<div class={clsx("w-full border-t-2 h-1/5 border-[#240465] flex flex-row cursor-pointer hover:bg-[#e0e0e2]", !chats[lastChat].opened && 'bg-[#e0e0e2] font-bold')} on:click={() => {openMessage(chats[0].chatId, chats[lastChat].messageId, chats[lastChat].opened)}}>
  <div class="p-2 w-1/5">{chats[lastChat].fromUser === userId ? "You" : chats[lastChat].username}</div>
